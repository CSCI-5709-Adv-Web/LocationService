...
export const getCoordinates = async (address: string): Promise<GeocodeResponse> => {
  return trackPerformance(
    "GetCoordinates",
    async () => {
      // Generate cache key
...
export const calculateRoute = async (fromAddress: string, toAddress: string): Promise<RouteData> => {
  return trackPerformance(
...
export const getDetailedRoute = async (fromAddress: string, toAddress: string): Promise<DetailedRouteData> => {
  return trackPerformance(
    ...
    async () => {
      // Generate cache key
      const cacheKey = generateCacheKey("detailedRoute", { fromAddress, toAddress })
